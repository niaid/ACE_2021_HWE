...
The directory should contain 30 .bam files, each with an index generated by Samtools. Each .bam file is the result of mapping low-coverage genome sequencing data from one individual to the human genome reference. The dataset contains 10 individuals from each of 3 geographically distant sites, indicated in the file names.

Let's take a look at a bam file
```bash
samtools view bamfiles/smallNA06994.mapped.ILLUMINA.bwa.CEU.low_coverage.20120522.bam | head
```
